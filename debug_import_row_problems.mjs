...
for (const sheetName of sheets) {
  ...
  for (let rowNumber = headerRowNumber + 1; rowNumber <= sheet.rowCount; rowNumber++) {
    ...
    if (entries.length > 1) {
      dupCount++;
      console.log('Row', rowNumber, 'multiple entries:', JSON.stringify(entries));
    }
    if (entries.length === 1 && entries[0].category === 'Miscellaneous' && categoryColIndices.length > 0 && !categoryVal) {
      console.log('Row', rowNumber, 'fallback misc:', JSON.stringify(entries));
    }
  }
  console.log('rows with multiple entries:', dupCount);
}
