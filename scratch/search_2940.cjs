...
async function searchAround2940() {
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    const total = inputVat + vatExempt;

    if (total >= 2930 && total <= 2950) {
      console.log(`Row ${rowNumber}: Date="${dateStr}" Product="${product}" InputVAT=${inputVat} VATExempt=${vatExempt} Total=${total}`);
    }
  });
}

searchAround2940().catch(console.error);
